...
# Архитектура ИТ-платформы: 3 слоя, 6 характеристик (ЭМОГСС) и принцип отчуждаемости
...
## Шесть архитектурных характеристик (ЭМОГСС)
...
**Вопрос:** Какова задержка от запроса пользователя до получения результата?
...
**Вопрос:** Соответствуют ли архитектурные решения актуальным SOTA-практикам?

**Как обеспечиваем:**
- SOTA-сущности формализованы в Pack (DP.SOTA.*) с аудируемым обновлением
- Приоритетная тройка (Context Engineering, DDD Strategic, Coupling Model) применяется к каждому решению
- 18 SOTA-практик покрывают полный конвейер: от захвата знаний до проекции в downstream

**Индикатор:** Проводилась ли SOTA-сверка при последнем архитектурном решении? Целевое: все решения проходят АрхГейт с оценкой современности.

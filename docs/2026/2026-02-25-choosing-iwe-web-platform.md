...
Сейчас единственная точка входа в нашу интеллектуальную рабочую среду (IWE) — Telegram-бот [@aist_me_bot](https://t.me/aist_me_bot). Бот уже умеет многое: отвечает на вопросы, показывает цифровой двойник, проводит тесты, ведёт марафоны, публикует в клуб. За ним стоят три MCP-сервера, три Pack'а с доменными знаниями, пять ИИ-агентов и база данных.
...
**Почему не прошёл как primary.** Discourse — отличная платформа для **сообщества** (человек и человек: 9/10). Но для **человек и ИИ** — слабо: sandbox ограничен 2 секундами на tool, нет structured output, нет per-user AI-памяти. Генеративность — 5: форум не создаёт платформу для AI-native фич.

**Но у Discourse есть роль.** Помимо сообщества (Phase 3), Discourse может стать **наблюдаемой доской для агентов**: Стратег, Экстрактор, Проводник работают на бэкенде (быстро, structured data), а ключевые моменты их взаимодействия публикуются в Discourse — чтобы человек мог видеть, как агенты критикуют и дополняют друг друга. Web App показывает agent activity feed для владельца, Discourse — для сообщества. Агенты работают на бэкенде, а Discourse и Web App — два окна в их работу.
...
**Web App — каноническая поверхность.** Работает в любом браузере, в любой стране, на любом языке. PWA даёт опыт, близкий к мобильному приложению. AI SDK v6 подключает все Pack-серверы нативно. Включает **agent activity feed** — владелец видит, как агенты взаимодействуют, критикуют и дополняют друг друга.
...
**Discourse — двойная роль (Phase 3).** Во-первых, платформа для сообщества (peer learning, обсуждения). Во-вторых, **наблюдаемая доска агентов**: агенты работают на бэкенде (быстро, structured data), а ключевые моменты их взаимодействия публикуются в Discourse — видимо для всех участников. Это не замена бэкенду — это окно в работу агентов для сообщества.
...
## UX-метафора: банковское приложение для развития

Когда я описываю Web App коллегам, лучше всего работает одна аналогия: **банковское приложение, только для интеллектуального капитала**.
...
| Push-уведомления | Инсайты из ЦД, напоминания |
| Личный кабинет | Профиль Цифрового двойника |
| Кешбэк / бонусные мили | Токены (баллы) за ДЗ, посты, вклад в экосистему |
| Программа лояльности | Статус участника, привилегии по накоплению токенов |

Банковское приложение создаёт привычку **проверять своё финансовое состояние** ежедневно. IWE Web App должен создать привычку **проверять своё состояние развития**: что освоено, что в процессе, куда двигаться дальше.

И ещё одна параллель: в банковском приложении вы видите **баланс бонусов** — кешбэк, мили, баллы лояльности. В IWE Web App у пользователя тоже есть баланс — **токены за активные действия**: выполнение домашних заданий, написание постов, помощь другим участникам, вклад в развитие экосистемы. Чем больше токенов — тем выше статус в программе лояльности. Это не геймификация ради геймификации — это отражение реального вклада человека в собственное развитие и развитие сообщества.
...
## Что уже есть, а что строится

Частый вопрос: «Придётся всё делать с нуля?» Нет. Web App — это **новый фронтенд** к существующей инфраструктуре. Как мобильное приложение банка: банк (серверы, база, логика) уже работает — приложение лишь новое окно к тем же данным.

**Уже работает и не меняется:**

| Компонент | Что делает |
|---|---|
| Neon DB | Все данные пользователей, ЦД, история |
| Railway (бэкенд) | Бизнес-логика, API, бот |
| MCP-серверы | Доступ к знаниям (Pack) и цифровому двойнику |
| GitHub | Хранилище репозиториев — Pack, DS, код |
| Claude API | AI-модель для ассистента |

**Строится заново (фронтенд + тонкий API-слой):**

| Компонент | Что делаем | Из готового |
|---|---|---|
| UI-компоненты | Кнопки, формы, карточки, layout | shadcn/ui (50+ готовых компонентов) |
| Графики прогресса | Визуализация ЦД | Recharts |
| AI-чат | Чат с ассистентом | AI SDK v6 (готовый useChat) |
| Авторизация | Вход и регистрация | NextAuth.js |
| Локализация | RU + EN | next-intl |
| **Бизнес-логика UI** | **Какие виджеты, как показать ЦД, токены, маршрут** | **Своё — это наше ядро** |

**Итого:** ~70% инфраструктуры уже существует. Строится UI + API-маршруты к существующему бэкенду.

Пользователь в Web App открывает дашборд → Web App запрашивает данные у бэкенда (Railway) → бэкенд читает из Neon DB и MCP → возвращает → Web App рисует красивый интерфейс. GitHub остаётся хранилищем для всех репозиториев.

...
source_knowledge:
  ...
  - DP.D.025
  - PD.FORM.018
tags: [AI, агенты, context-engineering, онбординг, аналогия, LLM]
content_plan: null
---

# ИИ-агенты: от модели к сотруднику

ChatGPT, Claude, Gemini — не агенты и не специалисты. Это обученные модели, знания которых похожи на знания выпускника вуза. Чтобы модель стала полезным агентом, её нужно запустить на железе, дать роль, знания вашего домена и контекст вашей ситуации. Всё как с наймом сотрудника — только быстрее, дешевле и масштабируемее.

---

## Что НЕ является ИИ-агентом

Начнём с распространённых ошибок.

**«Claude — это ИИ-агент».** Нет. Claude — это языковая модель (LLM). Набор обученных весов, файл на диске. Как книга с знаниями — пока никто не читает, она ничего не делает.

**«ChatGPT — это специалист».** Нет. ChatGPT — это сервис, который запускает модель на сервере и предоставляет интерфейс. Модель обладает знаниями, но не знает ничего о вас, вашей ситуации, ваших задачах.
...
Это определение работает безмасштабно: агентом может быть человек, робот, организация, ИИ-система. Ключевое свойство — не интеллект и не знания, а **способность действовать ради цели**.

Для того чтобы ИИ-агент существовал, нужны три условия одновременно:

1. **Железо** (hardware) — сервер с GPU, на котором работает вычисление. Это аналог тела: без него никакое действие невозможно.
2. **Модель** (software) — обученная нейросеть с знаниями. Это аналог знаний и навыков: ценный артефакт, но сам по себе — файл, который ничего не делает.
3. **Обвязка** (harness) — роль, контекст, инструменты, память, протоколы. Это аналог рабочего места: должность, доступы, документация, регламенты.
...
### Этап 1. Обучение (кто-то другой создал специалиста)

Выпускник экономического факультета провёл 4-6 лет в вузе. Преподаватели передали ему знания: МСФО, финмодели, статистику, экономическую теорию. Это **обучение** — длительный, дорогой процесс, который вы не контролируете.

**Аналог в ИИ: pre-training.** Компания (Anthropic, OpenAI, Google) обучает модель на огромном массиве текстов. Результат — модель, которая «знает» множество предметных областей. Вы не участвуете в этом процессе и не влияете на него. Вы получаете результат: модель с широкими, но не специфичными знаниями.
...
Иногда выпускник вуза проходит дополнительную программу: MBA, сертификация CFA, стажировка в Big Four. Это **дообучение** — специализация знаний под конкретную область. Не каждый работодатель организует это — но некоторые компании оплачивают сертификации новым сотрудникам.
...
| Человек | ИИ |
|---------|-----|
| Сертификация CFA после вуза | Fine-tuning модели на ваших данных |
| Меняет знания «в голове» (навсегда) | Меняет веса модели (навсегда в рамках этой версии) |
| Дорого, долго, результат предсказуем | Дорого, требует данных, результат не всегда предсказуем |
| Делает специалист / учебный центр | Делает ML-инженер / провайдер модели |

Большинство пользователей НЕ делают fine-tuning. Это как разница между «нанять выпускника МГУ» и «оплатить ему ещё год в Гарварде». Обычно в этом нет необходимости — достаточно хорошего онбординга.

### Этап 3. Найм (подписка / доступ)
...
**Аналог в ИИ: подписка или API.** Вы платите за доступ к модели. Claude Pro, ChatGPT Plus, API по токенам. Это не покупка — это аренда вычислительных мощностей и доступа к знаниям модели.
...
### Этап 4. Онбординг (ввод в должность)
...
И каждый понедельник — как первый рабочий день, потому что у него нет памяти о прошлых рабочих неделях.
...
|------------------------|-------------------------------|
| Должностная инструкция | **Роль** (системный промпт): какие методы применять, как себя вести |
...
**Аналог в ИИ: промпт.** Промпт — это не магическое заклинание. Это постановка задачи: что нужно сделать, в каком формате, с какими ограничениями.
...
|------|-----------|--------|----------|
| **Изготовитель** | Обучает базовую модель (pre-training) | Anthropic, OpenAI, Google | Вуз, который выпускает специалистов |
| **Дообучатель** | Специализирует модель (fine-tuning) | ML-инженер компании, провайдер | Учебный центр, который проводит сертификацию |
| **Инженер среды** | Проектирует обвязку (harness): промпты, инструменты, знания, протоколы | Разработчик ИИ-систем, инженер платформы | HR + руководитель, который создаёт рабочее место, регламенты, систему адаптации |
| **Пользователь** | Ставит задачи и принимает результаты | Конечный пользователь | Заказчик работы, который формулирует «что нужно» |
...
- Не знает ваши ограничения (бюджет, сроки, политику)
...
1. Кто-то должен её **обучить** (Anthropic, OpenAI — это «вуз»)
2. Иногда нужно **дообучить** под вашу специфику (fine-tuning — это «сертификация»)
3. Нужно **оформить доступ** (подписка — это «зарплата»)
4. Нужно **провести онбординг** (context engineering — это «ввод в должность»)
5. Нужно **ставить задачи** (промпт — это «техзадание»)
6. Нужно **принимать результат** (проверка — это «приёмка работы»)

Большинство людей останавливаются на шагах 3 и 5: оплатили подписку и пишут промпты. Пропускают шаг 4 — онбординг. И удивляются, что «ИИ не понимает мою ситуацию».

...
1. **Запоминание** — воспроизвести факт: «У насекомого шесть ног»
2. **Понимание** — объяснить своими словами: «Муха — насекомое, потому что...»
3. **Применение** — использовать знание в новой ситуации: определить, является ли паук насекомым
4. **Анализ** — разобрать на части и найти связи: что общего между слоном и мухой
5. **Оценка** — сравнить подходы и обосновать выбор: какая классификация полезнее для этой задачи
6. **Создание** — построить новое: спроектировать систему по принципам из биологии

Найти общее между двумя объектами — это уровень анализа, четвёртый из шести. Большинство образовательных программ систематически не поднимается выше третьего.

Если взять принципы мышления и разложить каждый по этим уровням, получится таблица. Строки — конкретные принципы: 6 нулевых (аксиоматичность, структура, многомасштабность, оптимизация, вероятность, ограничения) и 7 частей первых принципов (архитектура ядра, трансдисциплинарные рассуждения, спецификации, этика, governance, унификация, SoTA). Столбцы — уровни глубины по Блуму. Каждое пересечение — **ячейка**: «аксиоматичность на уровне анализа», «структура на уровне создания» и так далее.

Таких ячеек порядка 80+, потому что внутри каждой части первых принципов есть детализации: конкретные паттерны (холоны, ограниченные контексты, трансдукция...), каждый из которых тоже раскладывается по всем уровням глубины. В каждой ячейке описано конкретно: что ученик *может делать* на этой глубине, какие методы обучения работают, какие типичные ошибки, как проверить освоение. Это превращает расплывчатое «учить мыслить» в карту с координатами.

Но есть хорошая новость: это не талант. Это навык. Он тренируется — и карта для этого уже есть.
...
Уровни Блума — не единственная шкала когнитивной глубины. Есть SOLO-таксономия (Biggs & Collis, 1982) — пять уровней от «нет понимания» до «абстрактного переноса». Webb's Depth of Knowledge (1997) — четыре уровня сложности заданий. Модель Дрейфусов (1980) — пять стадий от новичка до эксперта. Таксономия Марзано (2001) — шесть уровней с явным метакогнитивным слоем.

Блум выбран по трём причинам.

**Операционализируемость.** Каждый уровень описывается через наблюдаемые действия: «может назвать», «может объяснить», «может применить», «может разобрать», «может оценить», «может создать». Это позволяет для каждой ячейки (принцип × глубина) сформулировать конкретное *can-do* — и проверить его тестом.

**Универсальность.** Блум работает для любого содержания — от физики до этики, от системного мышления до программирования. SOLO и Webb привязаны к типу задания, Дрейфус — к практическому навыку. Блум — к мышлению как таковому.

**Проверенность.** За 70 лет (1956–2026) это самый исследованный инструмент в дидактике. Пересмотренная версия Андерсона и Кратвола (2001) устранила основные критики оригинала, добавив измерение «тип знания» (фактическое, концептуальное, процедурное, метакогнитивное).

Альтернативы не отвергнуты — они используются для других задач. Модель Дрейфусов хороша для оценки практического мастерства. SOLO-таксономия полезна для оценки качества конкретного ответа. Но для построения учебной карты «принцип × глубина» Блум даёт наиболее чёткую сетку координат.
